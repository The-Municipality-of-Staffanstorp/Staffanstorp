...
add_image_size( 'large', 1440, 1024 );
}

/**** Enqueue jquery ****/ 
if( !is_admin()){
	wp_deregister_script('jquery');
	wp_register_script('jquery', ("http://ajax.googleapis.com/ajax/libs/jquery/1.4.2/jquery.js"), false, '1.4.2');
...
function fb_change_mce_buttons( $initArray ) {
	...
	$initArray['theme_advanced_disable'] = 'strikethrough,underline,forecolor,justifyfull,justifyleft,justifycenter,justifyright,indent,outdent,fullscreen';
	return $initArray;
}
...
if( !function_exists('base_extended_editor_mce_buttons') ){
	function base_extended_editor_mce_buttons($buttons) {
		...
		return array(
			'bold', 'italic', 'formatselect', 'bullist', 'numlist', 'blockquote', 'link', 'unlink', 'pastetext', 'pasteword', 'removeformat', 'charmap', 'undo', 'redo', 'wp_help'
		);
		/* WordPress Default
		return array(
...
/**** Excerpt length ****/
function new_excerpt_length($length) {return 36;}
...
function widgets_init_pj() {
	...
	'after_title' => '</h3><div class="hr"></div><hr />'));
	...
	register_sidebar(array(
	'name' => 'Öppettider',
	'before_widget' => '<li class="widget-container oppettider">',
	'after_widget' => '</li>',
	'description' => 'Widgetyta för ev. öppetider',
	'before_title' => '<h3 class="widget-title">',
	'after_title' => '</h3>'));
	...
	'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
	'after_widget' => '</li>',
	'description' => 'Widgetyta för etikettmoln på vänstersidan av sidfoten (4 kolumner bred)',
	'before_title' => '<h3 class="widget-title">',
	'after_title' => '</h3><div class="hr"></div>'));
...
/**** Usage: [epost]email@yourdomain.com[/epost] ****/
function munge_mail_shortcode( $atts , $content=null ) {
for ($i = 0; $i < strlen($content); $i++) $encodedmail .= "&#" . ord($content[$i]) . ';';
return '<a href="mailto:'.$encodedmail.'">'.$encodedmail.'</a>';}
add_shortcode('epost', 'munge_mail_shortcode');

...
<ul class="entry-meta">
<li class="entry-meta-datum">Datum <?php the_time('j F Y'); ?></li>
...
$kategorier = get_categories($katargs);

foreach($kategorier as $kat){
	$katID = $kat->cat_ID;
	$katTitel = $kat->cat_name;
	
	if ($katTitel == "Okategoriserade"){
		//echo 'sdfsdf';
	} else {
		echo '<h4 id="Blankett'. $katTitel .'">'.$katTitel.'</h4>';
	};
	
	$args = array(
		'post_type' => 'attachment', // search only attachments (media files)
		//'cat' => $katID, // from category
	...
	if ($filer) {
		...
		$filRubrik = $fil->post_excerpt;
		if($filRubrik == $katTitel){
			$filID = $fil->ID;
...
<?php if (get_post_meta($post->ID, 'Personuppgiftslagen', true) ) : ?>
<div class="hr"></div><hr />
<?php echo get_post_meta($post->ID, 'Personuppgiftslagen', true); ?>
<?php endif; ?>
